...
/**
 * @var adminID
 * @brief 4. Admin Telegram Chat Id. 
...
function addNewExpenseStep1Date(id)
{
  ...
  var keyBoard = {
    "inline_keyboard": [
      [
        {
          "text": "📅 Today - " + today.toLocaleDateString("en-US"),
          "callback_data": "newExpDate" + today.toLocaleDateString("en-US")
        }
      ],
      [
        {
          "text": "📆 Yestday - " + yesterday.toLocaleDateString("en-US"),
          "callback_data": "newExpDate" + yesterday.toLocaleDateString("en-US")
        }
      ],
      [
        {
          "text": "🗓️ 2 Days Ago - " + twoDaysAgo.toLocaleDateString("en-US"),
          "callback_data": "newExpDate" + twoDaysAgo.toLocaleDateString("en-US")
        },
      ]
    ]
  };
 ...
 *         false: Error
 */
function manageAddExpense(id, text)
{
    // Google Sheet
    var googleSheet = SpreadsheetApp.openById(ssId);
    var telegramLog = googleSheet.getSheetByName("Logs");
...
function deleteAnExpenseChoose(id)
{
  ...
  for (var i = lr; i > 0; i--)
  {
    ...
    categoriesList.push(
      [
        {
          "text": "🗑️ " + new Date(date).toLocaleDateString("en-US") + ": " + category + " - " + description + " - $ " + cost,
          "callback_data": "deleteExpense" + i + "_" + date + "_" + cost
        }
      ]
    );
    counter++;
...
function doPost(e)
{
  ...
  try
  {
    ...
    if (contents.callback_query) 
    {
      ...
      if (!checkUserAutentication(id_callback))
        return;
      ...
      {
        sendText(id_callback, "<b>🕰️ Date: " + data.slice("newExpDate".length) + "</b>");
    ...
    else if (contents.message) 
    {
      ...
      if (!checkUserAutentication(id_message))
        return;
